...
(function () {
  ...
  const VET_AIRWAY_RED_NARRATIVES = [
    ...
    (p, s) => `${s}. ${cap(p)} is found face-down. You roll them and reposition the airway — chest begins to rise on its own after about three seconds. Color returns slowly.`,
  ...
  const VET_EXTREMIS_NARRATIVES = [
    ...
    (p, s) => `${s}. ${cap(p)} on the ground with multi-system trauma. Tachypneic, cap refill ~5 sec, mumbling without purpose. Shock state — but airway is open and they're moving air.`,
