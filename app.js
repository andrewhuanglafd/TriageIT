...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('sp-start').addEventListener('click', () => {
    ...
    startGame({ deck: buildDeck(n), mode: 'solo' });
  });
  ...
  $('play-again').addEventListener('click', () => {
    if (state.game?.mode === 'mp-host') {
      ...
      startGame({ deck: buildDeck(state.spDeckCount), mode: 'solo' });
    }
  });
  ...
  html += players.map((p, i) => {
    ...
    return `<div class="lb-row ${rowCls}">
      ...
      <span class="nm">
        ...
        <span class="lb-meta">${teamTxt}${meTxt}${finishMark}</span>
      </span>
      <span class="sc">
...
async function hostStartGame() {
  ...
  const deck = buildDeck(state.mpDeckCount);
