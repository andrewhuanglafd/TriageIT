...
// Light-hearted F-grade ribbing — meant to land like locker-room
// banter, not a real review. Picked at random in endGame() and
// shown under the score on the AAR. Keep these short, witty, and
// firefighter/EMS-flavored without being mean.
const F_ROASTS = [
...
function endGame() {
  ...
  // F-grade ribbing — pick one line at random; hide otherwise.
  const roastEl = $('result-roast');
  if (roastEl) {
    if (grade === 'F') {
      roastEl.textContent = F_ROASTS[Math.floor(Math.random() * F_ROASTS.length)];
