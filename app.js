...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('sp-start').addEventListener('click', () => {
    ...
    // Both checkboxes can be active simultaneously. Combine into a single value.
    const hard   = $('sp-difficulty-hard')?.checked;
    const expert = $('sp-difficulty-expert')?.checked;
  ...
  // Difficulty checkboxes — host writes the combined value to the lobby
  // so guests see whichever modes are active.
  function syncMpDifficultyFromCheckboxes() {
    const hard   = $('mp-difficulty-hard')?.checked;
    const expert = $('mp-difficulty-expert')?.checked;
    setMpDifficulty(combinedDifficulty(hard, expert));
  }
  $('mp-difficulty-hard')?.addEventListener('change', syncMpDifficultyFromCheckboxes);
  $('mp-difficulty-expert')?.addEventListener('change', syncMpDifficultyFromCheckboxes);
...
function renderLobby() {
  ...
  // Each mode gets its own banner; both can show if stacked.
  ...
  // Sync host's difficulty checkboxes to lobby state
  const diffHard = $('mp-difficulty-hard');
  const diffExpert = $('mp-difficulty-expert');
  if (diffHard)   diffHard.checked   = diff.hard;
  if (diffExpert) diffExpert.checked = diff.expert;
}
