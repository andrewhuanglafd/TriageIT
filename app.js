...
function modeLabel(screenId) {
  ...
  if (screenId.startsWith('screen-gm-')) {
    const subBadge = ({ hard: '⚠ HARD', expert: '☠ EXPERT', chaos: '⚡ CHAOS', fog: '◐ FOG OF WAR', treat: '▥ TREATMENT AREA' })[gm.subMode];
...
function gmSyncSetupScreen() {
  ...
  // Difficulty: clear so the user picks fresh — chaos/fog/treat buttons
  // must be tapped explicitly to set gm.subMode. The breadcrumb falls
  // back to "MODE 02 · GAME MODE" until they do.
  ['gm-mode-chaos', 'gm-mode-fog', 'gm-mode-treat'].forEach(id => {
    const b = $(id);
    if (!b) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // End drill early — Training mode. Stops the round and grades against
  // what the player has actually answered so far. endGame() already
  // computes accuracy from g.correct + g.wrong for partial completions.
  $('training-end-early')?.addEventListener('click', () => {
    if (!state.game) return;
    if (confirm('End the drill now?')) endGame();
  });
  ...
  // Click-for-why modal: tapping a wrong placement on any visual
  // treatment area opens an info modal with the START rationale.
  // Listen on the document so the same handler works for the leaderboard,
  // the Training AAR, the Game Mode AAR, and the Treatment Area AAR.
  document.addEventListener('click', (e) => {
    const pip = e.target.closest('.treat-aar-pip[data-decision-idx]');
    if (!pip) return;
    const idx = parseInt(pip.dataset.decisionIdx, 10);
    if (isNaN(idx)) return;
    _lastTxClickedIdx = idx;   // remembered for the close-flash
    openClsWhyModal(idx);
  });
  $('cls-why-close')?.addEventListener('click', closeClsWhyModal);
  $('cls-why-modal')?.addEventListener('click', (e) => {
    if (e.target.id === 'cls-why-modal') closeClsWhyModal();
  });
...
function renderCard() {
  ...
  // SCOPED to Training's tag row. The unscoped `qsa('.t-btn')` used to
  // touch every .t-btn on the page (Fog of War, Hard, Treatment Area),
  // which left those buttons disabled between sessions.
  qsa('#screen-game .t-btn').forEach(b => b.disabled = false);
...
function handleDecisionTimerChange(newSecs) {
  ...
  const buttonsLive = qsa('#screen-game .t-btn').some(b => !b.disabled);
...
function onTagPick(tag) {
  ...
  // Scope to Training mode's tag row. An unscoped selector here used to
  // disable every .t-btn on the page (Fog of War, Hard, Treatment Area
  // tag rows are also .t-btn elements). The next time the player
  // entered a classroom Game Mode round, those buttons were still
  // disabled — clicks did nothing, every card timed out as wrong.
  qsa('#screen-game .t-btn').forEach(b => b.disabled = true);
  ...
  if ((g.mode === 'mp-host' || g.mode === 'mp-guest') && state.mpCode && state.mpMyId) {
    ...
    if (canTriggerEnd && meDone) {
      // Skip the instructor AND me explicitly. Defensive against any
      // stale state where instructor.finished isn't true yet. Matches
      // the Game Mode self-trigger pattern.
      const allDone = state.mpPlayers.every(p => {
        if (p.isInstructor) return true;
...
const GRADE_QUIPS = {
  S: [
    'Showed up, sorted the chaos, made the rest of us look bad. Quit being weird.',
    'Perfect run. Captain\'s mad about it. Don\'t bring it up at chow.',
    'Either you\'ve done this for real or you\'re a witch. We\'re voting witch.',
    'Either you\'re cheating or we\'re underpaying you. Probably both.',
    'Triage god status. Now go fold the rig laundry, hero.',
    'Probies are taking notes. Tell them to stop, you\'re ruining the curve.',
    'If you say one word about this score at chow I\'m dumping coffee on your gear.',
    'Clean sweep. The truck company is pretending they didn\'t see.',
    'S grade. Trophy goes on the kitchen shelf next to the BC\'s hot sauces.',
    'You triaged like you wrote the book. Stop showing off.',
  ],
  A: [
    'Almost perfect. We\'ll find the one wrong call and roast you for it for a week.',
    'Solid. Not gonna lie — kinda expected worse from you.',
    'An A. Don\'t let the engineer hear, he gets jealous easy.',
    'Real clean. Captain might let you ride right side this shift.',
    'You\'re the calm one. Annoying, but useful at 0300.',
    'Almost perfect — like your last station inspection. Almost.',
    'Top of the engine. You can have the recliner tonight.',
    'If this is your B-game, the rest of us should take up knitting.',
    'A grade. Engineer says quit making him look slow.',
  ],
  B: [
    'B grade. Brotherhood approves. Mostly. Don\'t push it.',
    'Got the job done. Like a Tuesday.',
    'Not flashy, not embarrassing. Like a properly tied bowline.',
    'Captain saw your score. He nodded once. That\'s parade-grade praise around here.',
    'Solid. Probies wish they were this boring.',
    'Above average. Below show-off. The sweet spot for not getting hazed.',
    'B for "better than the truck guy expected."',
    'Reliable. The captain likes reliable. The captain also likes coffee. Take notes.',
  ],
  C: [
    'Passing. Barely. Like your last apparatus check.',
    'Showed up. Did something. Drove the rig home in one piece. We\'ll take it.',
    'C-grade triage means most of them lived. Probably. Mostly.',
    'Average. Nobody\'s writing home about it. Not even your mom.',
    'Captain wants to see you. He\'s not mad. Just disappointed. Worse.',
    'Middle of the pack. The pack is currently napping.',
    'C means competent. It also means complacent. Pick a lane.',
    'The score that gets you "we\'ll work on it" from the BC. Bring a notebook.',
  ],
  D: [
    'Cleared the bar. Tripped over it. Took the bar with you.',
    'D for "did the bare minimum." Also "don\'t quit your day job."',
    'Almost an F. Instructor was feeling charitable. Don\'t bank on it.',
    'BC asked who trained you. We didn\'t have the heart to answer.',
    'You\'d be better off flipping a coin. Cheaper, too.',
    'D-grade. Like the engineer\'s coffee. Technically caffeinated, technically a beverage.',
    'Probie levels. The probie is mad we said that.',
    'Refresher course has your name on it. Bring lunch.',
    'You triaged like you were guessing. Because you were.',
  ],
  F: [
    'Good news: it was a drill. Bad news: the patients are still mad.',
    'Captain saw the score and signed you up for swift water class. Sink or swim, kid.',
    'Your patients self-triaged. Some of them voted, too.',
    'If this were real, the engineer would\'ve backed the rig over you.',
    'Probies are studying your run to learn what NOT to do. Get a print.',
    'Dispatch wants to know if you\'d like to be transferred. To dispatch.',
    'S.T.A.R.T. is the acronym. Not "pick at random and pray."',
    'You triaged like you were ordering off a menu in a language you don\'t speak.',
    'Captain\'s turning your run into a training video. The "do not" kind.',
    'Couldn\'t find the right tag with a GPS and a map.',
    'BC wants you to do every refresher in the catalog. He brought a list. It\'s long.',
    'Even the walking wounded started directing themselves. They had time to vote.',
    'After-action report is just the captain rubbing his forehead.',
    'Your scorecard is going in the firehouse hall of shame. Bring a frame.',
    'Chief wants you to write "Sort, don\'t treat" 100 times. With your weak hand.',
    'Truck guys are calling you "Coach." It\'s not a compliment.',
    'You set triage back about three decades. Congrats on the time travel.',
  ],
};
...
function endGame() {
  ...
  // Always grade against cards-actually-answered. This handles all three
  // end conditions cleanly: full completion (answered == deck.length),
  // time-bound expiry (answered < deck.length), and END DRILL EARLY
  // (answered < deck.length). Without this, an early-end in cards-bound
  // would show "5/25 = 20%" instead of "5/5 = 100%" for a clean partial.
  ...
  // MP mode: write the final stats + finished flag here. This covers:
  //   * time-bound rounds where the per-card writer's isLastCard branch
  //     never fired (timer ended the run before the last card),
  //   * END DRILL EARLY in cards-bound rounds (also short-circuits the
  //     last-card write — without this, Firebase keeps stale wrong/correct
  //     numbers and the leaderboard shows "perfect score" symptoms).
  //   * Normal cards-bound completion (idempotent — same values get re-written).
  if ((g.mode === 'mp-host' || g.mode === 'mp-guest') && state.mpCode && state.mpMyId) {
    const updates = {};
    updates[`players/${state.mpMyId}/correct`]   = g.correct;
    updates[`players/${state.mpMyId}/wrong`]     = g.wrong;
    ...
    updates['lastActivity'] = firebase.database.ServerValue.TIMESTAMP;
    fbLobbyRef(state.mpCode).update(updates).catch(() => {});
    // Last-player end-of-round trigger. Skip the instructor AND me
    // explicitly so this can't get stuck on a stale instructor record.
    const isClassroom = state.mpLobbyType === 'classroom';
    const canTriggerEnd = (g.mode === 'mp-host') || (isClassroom && g.mode === 'mp-guest');
    if (canTriggerEnd) {
      const allDone = state.mpPlayers.every(p => {
  ...
  // Visual treatment area — every patient placed at the tarp the player
  // chose. Wrong placements are tappable for the why-modal. Built from
  // the player's local perCard history via getMyDecisions().
  const txEl = $('result-tx-area');
  if (txEl) {
    txEl.innerHTML = renderTreatmentAreaHtml(getMyDecisions());
  }
...
function renderClsLeaderboard() {
  ...
  if (sub) sub.textContent = `${finishedCount} of ${totalPlayers} finished · ${(state.mpGameType || 'training').toUpperCase()}`;

  const me = sorted.find(p => p.id === state.mpMyId);
  const myRank = me ? (sorted.indexOf(me) + 1) : null;

  let html = '';

  // === PODIUM (top 3) — visible to everyone ===
  html += `<div class="lb-podium">`;
  // Render in order 2nd, 1st, 3rd so the gold sits center-front.
  const podiumOrder = [sorted[1], sorted[0], sorted[2]];
  const podiumPositions = [2, 1, 3];
  podiumOrder.forEach((p, i) => {
    const place = podiumPositions[i];
    if (!p) {
      html += `<div class="podium-slot place-${place} empty"><span class="podium-empty">—</span></div>`;
      return;
    }
    ...
    const trophy = ['', '🥇', '🥈', '🥉'][place];
    const youBadge = (p.id === state.mpMyId) ? `<span class="podium-you">YOU</span>` : '';
    html += `<div class="podium-slot place-${place}">
      <div class="podium-trophy">${trophy}</div>
      <div class="podium-name">${escapeHtml(p.name)}${youBadge}</div>
      <div class="podium-score">${correctOf}</div>
      <div class="podium-acc">${acc}%</div>
    </div>`;
  });
  html += `</div>`;

  // === "YOU" callout — non-instructor, not in top 3, has finished ===
  if (!isInstructor && me && myRank > 3) {
    const total = (me.correct || 0) + (me.wrong || 0);
    const correctOf = total > 0 ? `${me.correct}/${total}` : `${me.correct || 0}`;
    const acc = total > 0 ? Math.round((me.correct / total) * 100) : 0;
    html += `<div class="lb-you-card">
      <div class="lb-you-tag">YOU</div>
      <div class="lb-you-rank">#${myRank} <span class="lb-you-of">of ${totalPlayers}</span></div>
      <div class="lb-you-stats">${correctOf} · ${acc}%</div>
    </div>`;
  } else if (!isInstructor && me && !me.finished) {
    html += `<div class="lb-you-card waiting">
      <div class="lb-you-tag">YOU</div>
      <div class="lb-you-rank">— <span class="lb-you-of">still triaging</span></div>
    </div>`;
  }

  // === Player view: visual treatment area ===
  // Works across all classroom round types. renderPlayerTreatmentAreaHtml
  // pulls from getMyDecisions() which knows about Training (state.game),
  // Game Mode (gm.history), and Treatment Area (gm.treat.history).
  if (!isInstructor && me) {
    html += renderPlayerTreatmentAreaHtml(me);
  }

  // === Instructor view: full ranking list (positions 4..N) ===
  // Top 3 are already in the podium; this is the rest of the class
  // for the instructor only.
  if (isInstructor && sorted.length > 3) {
    html += `<h3 class="lb-rest-h">REST OF CLASS</h3><div class="lb-rest-list">`;
    sorted.slice(3).forEach((p, i) => {
      ...
      const acc = total > 0 ? Math.round((p.correct / total) * 100) : 0;
      const finishMark = p.finished ? '' : `<span class="lb-status">in-progress</span>`;
      html += `<div class="lb-rest-row">
        <span class="rest-rank">#${rank}</span>
        <span class="rest-name">${escapeHtml(p.name)}${finishMark}</span>
        <span class="rest-score">${correctOf}</span>
        <span class="rest-acc">${acc}%</span>
        <span class="rest-time">${fmtTime(p.totalTime)}</span>
      </div>`;
    });
    html += `</div>`;
  }
  ...
  const debriefEl = $('lb-instructor-debrief');
  if (toggleBtn) {
    toggleBtn.style.display = isInstructor ? '' : 'none';
    toggleBtn.textContent = '▼ EXPAND DEBRIEF · COMMONLY MISSED BY AREA';
  }
  if (debriefEl) debriefEl.style.display = 'none';
}

// Normalize the local player's per-card decisions into a single shape
// the visualization can consume. Source priority is screen-aware so
// stale state from a previous round can't leak into the current AAR:
//   * screen-gm-results → gm.treat.history (treat) or gm.history (other GM)
//   * screen-results / leaderboard → state.game first, then gm fallbacks
// Each entry: { card, picked, isCorrect, idx } where `card` has the
// standard scenario shape (description/respirations/perfusion/mental/
// answer/rationale). idx is a STABLE local index used by the why-modal.
function getMyDecisions() {
  const fromTraining = () => {
    if (!state.game || !Array.isArray(state.game.perCard) || !state.game.deck) return [];
    const out = [];
    state.game.perCard.forEach((c, i) => {
      if (!c || !c.picked) return;
      const card = state.game.deck[i];
      if (!card) return;
      out.push({ card, picked: c.picked, isCorrect: !!c.correct, idx: out.length });
    });
    return out;
  };
  const fromGameMode = () => {
    if (!gm || !Array.isArray(gm.history) || gm.history.length === 0) return [];
    const out = [];
    gm.history.forEach(h => {
      if (!h || !h.picked || !h.scenario) return;
      const truth = h.finalAnswer || h.originalAnswer || h.scenario.answer;
      const card = Object.assign({}, h.scenario, { answer: truth });
      out.push({ card, picked: h.picked, isCorrect: (h.type === 'correct'), idx: out.length });
    });
    return out;
  };
  const fromTreatMode = () => {
    if (!gm || !gm.treat || !Array.isArray(gm.treat.history) || gm.treat.history.length === 0) return [];
    const out = [];
    gm.treat.history.forEach(h => {
      if (!h || !h.patient || !h.picked) return;
      const card = Object.assign({}, h.patient.scenario, { answer: h.patient.correctTag });
      out.push({ card, picked: h.picked, isCorrect: !!h.correct, idx: out.length });
    });
    return out;
  };
  ...
  // The Game Mode AAR — must use gm data, never state.game (which may
  // hold stale Training perCard from a prior round in the same session).
  if (screen === 'screen-gm-results') {
    if (gm && gm.subMode === 'treat') {
      const out = fromTreatMode(); if (out.length) return out;
    }
    const out = fromGameMode(); if (out.length) return out;
    return [];
  }

  // Training AAR + classroom leaderboard — Training takes priority,
  // then fall back to whichever Game Mode source has data (covers
  // classroom Game Mode rounds that share the leaderboard screen).
  let out = fromTraining();   if (out.length) return out;
  out = fromTreatMode();      if (out.length) return out;
  out = fromGameMode();       if (out.length) return out;
  return [];
}

// In-memory cache so the click-for-why modal can look up cards by the
// stable local idx written on each pip's data-decision-idx attribute.
// Set whenever we render a treatment-area visualization.
let _lastTxDecisions = [];

// Inline SVG silhouette used as a visual anchor next to the patient
// label on every pip. Single neutral icon (head + shoulders), tinted
// via currentColor so it picks up the pip's text color automatically.
const PATIENT_SILHOUETTE_SVG = `<svg class="pip-silhouette" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
  <circle cx="12" cy="7.5" r="3.6" fill="currentColor"/>
  <path d="M3.5 22c0-4.7 3.8-8.5 8.5-8.5s8.5 3.8 8.5 8.5z" fill="currentColor"/>
</svg>`;

// Generalized "Your Treatment Area" board renderer. Takes a normalized
// decisions array (see getMyDecisions) and builds the 4-zone visual.
// Wrong placements get clickable pips that open the why-modal.
function renderTreatmentAreaHtml(decisions, opts) {
  opts = opts || {};
  const heading = opts.heading || 'YOUR TREATMENT AREA';
  const showHint = opts.showHint !== false;
  if (!decisions || decisions.length === 0) return '';

  // Cache decisions for the click-for-why lookup
  _lastTxDecisions = decisions;

  const placements = { red: [], yellow: [], green: [], black: [] };
  decisions.forEach(d => {
    if (placements[d.picked]) placements[d.picked].push(d);
  });

  const zoneHtml = (z, label) => {
    const items = placements[z];
    // The silhouette IS the status indicator: green for correct,
    // red for wrong. Wrong pips also slowly pulse.
    const pips = items.length === 0
      ? `<div class="treat-aar-empty">— empty —</div>`
      : items.map(it => {
          const cls = it.isCorrect ? 'correct' : 'wrong';
          return `<div class="treat-aar-pip ${cls}" data-decision-idx="${it.idx}" tabindex="0" role="button" aria-label="Show correct answer">
            ${PATIENT_SILHOUETTE_SVG}
            <span class="pip-name">${escapeHtml(treatShortNameFromCard(it.card))}</span>
          </div>`;
        }).join('');
    return `<div class="treat-aar-zone z-${z}">
      <div class="treat-aar-zone-head">${label} <span class="ct">${items.length}</span></div>
      <div class="treat-aar-zone-body">${pips}</div>
    </div>`;
  };

  const hintHtml = showHint
    ? `<div class="lb-tx-hint">Tap any patient to see the correct answer</div>`
    : '';
  const headingHtml = heading
    ? `<h3 class="lb-tx-h">${escapeHtml(heading)}</h3>`
    : '';

  return `
    ${headingHtml}
    <div class="treat-aar-board lb-tx-board">
      ${zoneHtml('red',    'IMMEDIATE')}
      ${zoneHtml('yellow', 'DELAYED')}
      ${zoneHtml('green',  'MINOR')}
      ${zoneHtml('black',  'DECEASED')}
    </div>
    ${hintHtml}
  `;
}

// Build a decisions array for the INSTRUCTOR's class-wide commonly-missed
// view. Each missed card is placed at its CORRECT tarp (picked = answer)
// and gets `isCorrect: false` so the renderer flashes it as "this one
// caused trouble." Carries class-wide miss metadata so the why-modal
// can show who missed and what they picked.
function buildClassMissesDecisions(deck, missesByCard, totalPlayers) {
  if (!deck || deck.length === 0) return [];
  const out = [];
  Object.keys(missesByCard).forEach(k => {
    const idx = parseInt(k, 10);
    const card = deck[idx];
    if (!card) return;
    const missList = missesByCard[idx] || [];
    if (missList.length === 0) return;
    out.push({
      card,
      picked: card.answer,        // renders in the CORRECT tarp
      isCorrect: false,           // flashes in the visualization
      idx: out.length,
      _isClassView: true,
      _missList: missList,
      _missCount: missList.length,
      _totalPlayers: totalPlayers || 1,
      _deckIdx: idx,
    });
  });
  // Sort by miss count descending — most-missed cards appear first
  // within their zones (flexbox preserves insertion order).
  out.sort((a, b) => b._missCount - a._missCount);
  return out;
}

// Backwards-compat wrapper kept for the existing call site. Builds
// decisions from the local player + state.mpDeck path, then renders.
function renderPlayerTreatmentAreaHtml(me) {
  // Prefer the unified decisions source (works for all modes); fall back
  // to the deck+misses path if the player's own state isn't available
  // (shouldn't happen for the local player, but defensive).
  let decisions = getMyDecisions();
  if (decisions.length === 0 && state.mpDeck && me) {
    const missByIdx = {};
    Object.values(me.misses || {}).forEach(m => {
      if (m && typeof m.idx === 'number') missByIdx[m.idx] = m;
    });
    const reached = (me.progress || 0);
    decisions = [];
    state.mpDeck.forEach((card, idx) => {
      if (idx >= reached) return;
      const miss = missByIdx[idx];
      const picked = miss ? miss.picked : card.answer;
      const isCorrect = !miss;
      decisions.push({ card, picked, isCorrect, idx: decisions.length });
    });
  }
  return renderTreatmentAreaHtml(decisions);
}

// Open the click-for-why modal for a given decision index (the local
// idx written on the pip's data-decision-idx attribute by
// renderTreatmentAreaHtml). Looks up the cached decision and shows:
//   * personal view: YOU PICKED + CORRECT ANSWER + rationale
//   * class view (instructor): CORRECT ANSWER + rationale + WHO MISSED
function openClsWhyModal(decisionIdx) {
  const d = _lastTxDecisions && _lastTxDecisions[decisionIdx];
  if (!d) return;
  const card = d.card;
  const picked = d.picked;
  const truth  = card.answer;
  const isClassView = !!d._isClassView;

  const t = (k) => (window.TRIAGE_INFO ? window.TRIAGE_INFO[k]?.label : (k || '').toUpperCase()) || k;
  const setIf = (id, txt, cls) => {
  ...
  // YOU PICKED row — hide in class view (it would just say "you picked
  // the right answer" since the pip was placed at the correct tarp).
  const pickedRow = document.querySelector('#cls-why-modal .why-header-row:first-child');
  if (pickedRow) pickedRow.hidden = isClassView;
  if (!isClassView) {
    setIf('cls-why-picked', t(picked), `why-header-v tag-${picked}`);
  }
  setIf('cls-why-truth',  t(truth),  `why-header-v truth tag-${truth}`);

  const narEl = document.getElementById('cls-why-narrative');
  if (narEl) narEl.textContent = card.description || '—';

  const vitalsEl = document.getElementById('cls-why-vitals');
  if (vitalsEl) {
    vitalsEl.innerHTML = `
      <div><span class="k">RESPIRATIONS</span><span class="v">${escapeHtml(card.respirations)}</span></div>
      <div><span class="k">PERFUSION</span><span class="v">${escapeHtml(card.perfusion)}</span></div>
      <div><span class="k">MENTAL STATUS</span><span class="v">${escapeHtml(card.mental)}</span></div>
    `;
  }

  const ratEl = document.getElementById('cls-why-rationale');
  if (ratEl) ratEl.textContent = card.rationale || '';

  // Class-view extras: who missed + what they picked.
  const missesEl = document.getElementById('cls-why-misses');
  if (missesEl) {
    if (isClassView && d._missList && d._missList.length) {
      const total = d._totalPlayers || d._missList.length;
      const pct = Math.round((d._missCount / total) * 100);
      const countEl = document.getElementById('cls-why-misses-count');
      if (countEl) countEl.textContent = `${d._missCount}/${total} (${pct}%)`;
      const listEl = document.getElementById('cls-why-misses-list');
      if (listEl) {
        listEl.innerHTML = d._missList.map(m =>
          `<span class="why-miss-pill"><span class="why-miss-name">${escapeHtml(m.name)}</span> picked <span class="why-miss-tag tag-${m.picked}">${escapeHtml(t(m.picked))}</span></span>`
        ).join('');
      }
      missesEl.hidden = false;
    } else {
      missesEl.hidden = true;
    }
  }

  const overlay = document.getElementById('cls-why-modal');
  if (overlay) overlay.hidden = false;
}

function closeClsWhyModal() {
  const overlay = document.getElementById('cls-why-modal');
  if (overlay) overlay.hidden = true;
  // Briefly flash the pip the user just inspected so they can see
  // which placement they came from after the modal disappears.
  if (typeof _lastTxClickedIdx === 'number') {
    const pip = document.querySelector(`.treat-aar-pip[data-decision-idx="${_lastTxClickedIdx}"]`);
    if (pip) {
      pip.classList.add('recently-viewed');
      setTimeout(() => pip.classList.remove('recently-viewed'), 1000);
    }
    _lastTxClickedIdx = null;
  }
}
let _lastTxClickedIdx = null;

// Short patient handle in EMS-standard "30 y/o M" format. Accepts a
// raw card (uses .description). Resolves both numeric ages
// ("approximately 70", "in his 40s") AND descriptive categories
// ("teenage", "middle-aged", "elderly") to a specific age. Age is
// deterministic per-description so the same patient always reads
// with the same age across re-renders.
function treatShortNameFromCard(card) {
  const d = card.description || '';
  const sex = sexFromNarrative(d);
  const age = ageFromNarrative(d);
  return age != null ? `${age} y/o ${sex}` : `Adult ${sex}`;
}

// Derive a sex letter from the description. If the narrative names a
// sex (man/woman/male/female/boy/girl) we use that; otherwise we
// deterministically pick M/F from the description hash so the same
// patient always reads the same way (e.g. "a delivery driver" stays
// the same sex across renders).
function sexFromNarrative(desc) {
  const d = desc || '';
  const sexMatch = d.match(/\b(man|woman|boy|girl|male|female)\b/i);
  if (sexMatch) return /^(?:man|male|boy)$/i.test(sexMatch[1]) ? 'M' : 'F';
  return (_ageHash(d) % 2 === 0) ? 'M' : 'F';
}

// Cheap deterministic hash so the age picked for a given card is stable
// across renders. Same description → same age every time.
function _ageHash(s) {
  let h = 0;
...
function _ageInRange(seed, lo, hi) {
  return lo + (_ageHash(seed) % (hi - lo + 1));
}

// Pull a specific age out of a narrative string. Recognized forms,
// in order of specificity:
//   "approximately 70"        → 70
//   "in his late 60s"         → 67-69
//   "in her early 30s"        → 30-33
//   "in his 40s"              → 40-49
//   "late 20s"                → 27-29
//   "early 30s"               → 30-33
//   "teenage" / "teen-aged"   → 13-19
//   "middle-aged"             → 40-65
//   "elderly"                 → 65-85
//   "older man/woman"         → 60-75
//   "young adult" / "college" → 18-25
//   "adult"                   → 25-55  (fallback)
// Returns null if nothing matches.
function ageFromNarrative(desc) {
  if (!desc) return null;
  const d = desc.toLowerCase();

  // 1. Specific number after "approximately"
  let m = d.match(/\bapproximately\s+(\d{2,3})/);
  if (m) return parseInt(m[1], 10);

  // 2. "in his/her late/early N0s"
  m = d.match(/\bin (?:his|her)\s+late\s+(\d0)s\b/);
  if (m) { const n = parseInt(m[1], 10); return _ageInRange(desc, n + 7, n + 9); }
  m = d.match(/\bin (?:his|her)\s+early\s+(\d0)s\b/);
  if (m) { const n = parseInt(m[1], 10); return _ageInRange(desc, n, n + 3); }

  // 3. "in his/her N0s"
  m = d.match(/\bin (?:his|her)\s+(\d0)s\b/);
  if (m) { const n = parseInt(m[1], 10); return _ageInRange(desc, n, n + 9); }

  // 4. "late/early N0s" (without "in his/her" prefix)
  m = d.match(/\blate\s+(\d0)s\b/);
  if (m) { const n = parseInt(m[1], 10); return _ageInRange(desc, n + 7, n + 9); }
  m = d.match(/\bearly\s+(\d0)s\b/);
  if (m) { const n = parseInt(m[1], 10); return _ageInRange(desc, n, n + 3); }

  // 5. Descriptive categories
  if (/\bteen(?:age|aged|ager)?\b/.test(d))                 return _ageInRange(desc, 13, 19);
  if (/\bmiddle[- ]aged\b/.test(d))                          return _ageInRange(desc, 40, 65);
  if (/\belderly\b/.test(d))                                 return _ageInRange(desc, 65, 85);
  if (/\bolder\s+(?:man|woman|adult|patient)\b/.test(d))     return _ageInRange(desc, 60, 75);
  if (/\byoung adult\b|\bcollege[- ]aged\b/.test(d))         return _ageInRange(desc, 18, 21);
  if (/\bdelivery driver\b/.test(d))                         return _ageInRange(desc, 30, 40);

  // 6. Default for plain "a man" / "a woman" / "male" / "female" with
  // no other age qualifier — treat as 30s. This catches narratives
  // like "a man in business attire" or "a woman wearing scrubs".
  if (/\b(?:man|woman|male|female)\b/i.test(d))              return _ageInRange(desc, 30, 39);
...
function renderDebrief() {
  ...
  // === COMMONLY MISSED QUESTIONS — GROUPED BY TREATMENT AREA ===
  // Aggregate misses across all players, group by card index, count
  // occurrences, then GROUP THOSE GROUPS by the card's correct tag
  // (the area the patient should've gone to). Uses Object.values to
  // handle both array- and object-shaped misses (Firebase quirk).
  const missesByCard = {};
  players.forEach(p => {
    Object.values(p.misses || {}).forEach(m => {
  ...
  if (flaggedIndices.length > 0 && state.mpDeck) {
    ...
    // Each commonly-missed card is rendered as a pip in its CORRECT
    // tarp (the area it should have gone to). The renderer flashes
    // them red because they're flagged as isCorrect:false. Tap any
    // pip to see who missed it, what they picked, and the rationale.
    const classDecisions = buildClassMissesDecisions(state.mpDeck, missesByCard, totalPlayers);
    html += `<h2 class="section-title">COMMONLY MISSED · CLASS TREATMENT AREA</h2>`;
    html += renderTreatmentAreaHtml(classDecisions, {
      heading: '',  // already shown by section-title above
      showHint: false,
    });
    html += `<div class="debrief-class-hint">Tap any flashing pip to see who missed it, what they picked, and the START rationale.</div>`;
  } else if (flaggedIndices.length === 0) {
    html += `<div class="debrief-clean">PERFECT ROUND · CAPTAIN IS SUSPICIOUS</div>`;
  }
...
function showReviewScreen(modeOverride) {
  ...
  if (indices.length === 0) {
    toast('Nothing to review. Suspiciously clean. The probie hates you.');
    return;
  }
...
// Treatment Area scoring — standalone retriage drill. Standard
// "correct vs wrong" doesn't fit because every retag is also a
// possible *fix* of a first-unit mistag, and YELLOW deterioration
// adds a "catch it before it tips" bonus axis.
const TREAT_SCORE = {
  catchDeterioration:    10, // upgraded a deteriorating yellow → red BEFORE becomesAt
  missDeterioration:    -15, // yellow tipped to red AND died on board (no transport, no upgrade)
  correctDowngrade:       5, // mistagged red → correct yellow
  wrongDowngrade:       -20, // real red → yellow (under-triage; the deadly mistake)
  correctUpgrade:         5, // any other correct upgrade (yellow→red on a real red, etc.)
  wrongUpgrade:         -10, // real yellow → red (over-triage panic)
  fixGreenMistag:         5, // restored a mistagged green back to green
  fixBlackMistag:         5, // restored a mistagged black back to black
  brokeCorrectTag:      -10, // touched a correct tag and made it wrong (any direction)
  spuriousChange:        -5, // changed an already-wrong tag to a different wrong tag
  noOp:                   0, // re-tagged with the same color (no penalty for looking)
  transportPriorityRed:   5, // first transport was a (correctly tagged) RED
  transportWrongTag:    -10, // transported a patient whose currentTag != correctTag (wasted resource)
  transportCorrect:       2, // transported any patient whose currentTag == correctTag (small reward)
};

// Treatment Area engine constants
const TREAT_DEFAULT_SCENE_MS    = 240000;  // 4 min default round
const TREAT_DETERIORATE_MIN_MS  = 30000;   // earliest a yellow can tip (30s grace)
const TREAT_DETERIORATE_MAX_MS  = 90000;   // latest tip
const TREAT_RED_EXPIRES_MS      = 35000;   // a RED dies this long after becoming red (matches GM_DETERIORATION.red)
const TREAT_AMBULANCE_INTERVAL_MS = 30000; // one ambulance arrives every 30s
const TREAT_TICK_MS             = 250;
...
const gm = {
  ...
  fogPickHandle: null,
  fogCurrent: null,
  // Treatment Area mode — see freshTreatState()
  treat: null,
};

function freshTreatState() {
  return {
    queue: [],                // ordered list of patients (each w/ correctTag, initialTag, scenario)
    currentIndex: 0,          // pointer into queue
    correct: 0,               // running counts
    wrong: 0,
    placements: { red: [], yellow: [], green: [], black: [] }, // per-zone history of placed patient records (for AAR)
    history: [],              // per-card events: {patient, picked, correct, atMs, type:'agree'|'fix'|'broke'|'missed'|'still_wrong'}
    elapsedHandle: null,      // setInterval handle for the elapsed-time HUD
    sceneSize: 12,
  };
}
...
function gmReset() {
  ...
  gm.fogPhase = null;
  gm.fogCurrent = null;
  // Stop the treatment-area elapsed-time ticker before recreating state
  if (gm.treat && gm.treat.elapsedHandle) {
    clearInterval(gm.treat.elapsedHandle);
  }
  gm.treat = freshTreatState();
...
function gmStartGame() {
  switch (gm.subMode) {
    ...
    case 'treat':  gmStartTreat();  return;
...
const CHAOS_SURVIVE_QUIPS = [
  'Survived with {N}. Captain says don\'t get cocky. Then he got cocky for you.',
  'Cleared {N} clean. Truck guys are pretending they\'re not impressed.',
  '{N} correct, zero deaths. Suspiciously good. We\'re checking the tape.',
  'Made it out alive. Buy the engineer dinner. He\'s owed it.',
  '{N} triages, no fails. The probie isn\'t speaking to you out of jealousy.',
  'Captain\'s looking at you funny. That\'s "good job" in his language.',
  'Don\'t celebrate. The next round is harder. So is the captain.',
  '{N} clean. The kind of run that ends arguments. Briefly.',
  'Survived. Don\'t get used to it — the firehouse never lets a good run stand.',
  '{N} for {N}. Either you trained hard or you got lucky. We\'re assuming lucky.',
  'Cleared the round. BC is suspicious. He\'s always suspicious.',
];

const CHAOS_DEATH_QUIPS = [
  'Made it to {N}. Crew is unimpressed. The probie laughed.',
  '{N} correct, then the wheels fell off. Engineer charges by the wheel.',
  'Sailed through {N}, then panicked like a probie at a 2-alarm.',
  '{N} cards. Then the brain went to lunch and forgot to come back.',
  'Could\'ve kept going. Didn\'t. The captain noticed. He always notices.',
  '{N} for {N}. Then a coin flip. You lost it.',
  'Probie levels: {N}. Captain levels: not happening tonight.',
  'You\'re great at the easy ones. The hard one ate your lunch.',
  '{N} clean, then one call ended your shift early. Reality doesn\'t grade on a curve.',
  'BC\'s writing it up as a "developmental opportunity." Be very afraid.',
  'Made it further than the truck guy\'s last run. Lower bar than you think.',
  '{N} cards. Crew\'s polite. The probie is not.',
  'Could\'ve sworn you knew this one. The vitals card disagrees.',
  '{N} good calls. Then a call so bad we\'re writing a poem about it.',
  'Made it to {N}, then folded faster than a station-issue cot.',
  '{N} cards. The captain put your name on the next refresher. He brought a highlighter.',
];
...
function gmFogShowPick(overrideMs) {
  ...
  if (!p || !gm.active) return;
  gm.fogPhase = 'pick';
  // Defensive: re-enable the Fog tag buttons every time the pick phase
  // opens. Guards against stale disabled state from a previous session
  // (e.g. a Training round on the same browser earlier).
  qsa('#screen-gm-fog .t-btn').forEach(b => b.disabled = false);
...
/* ============================================================
   TREATMENT AREA — pool-first / training-style retriage drill.
   Patients arrive ALREADY tagged by a "first unit." Player works
   through the queue one at a time, reassessing each and choosing
   the correct tag. The patient is then placed at the matching tarp.
   No clock — self-paced like Training Mode. End-of-round AAR shows
   the final treatment-area layout + missed cards.
   ============================================================ */

// Queue size for time-bound rounds — we generate a generous pool
// upfront so the player can keep going as long as the clock allows.
// Realistically nobody clears more than ~40 in 5 minutes; 100 is
// plenty of headroom.
const TREAT_TIME_QUEUE_SIZE = 100;

function gmStartTreat() {
  gmReset();
  gm.subMode = 'treat';
  gm.active = true;
  gm.classroomMode = false;          // standalone only in v1
  gm.startTime = Date.now();
  gm.difficulty = 'hard-expert';     // borderline vitals + red herrings — feels right for retriage
  gm.treat = freshTreatState();

  // Honor the picker. For cards-bound, queue size = cardLimit.
  // For time-bound, generate a big pool and end on the clock.
  const isTime = (gm.bound === 'time');
  const queueSize = isTime ? TREAT_TIME_QUEUE_SIZE : (gm.cardLimit || 25);
  gm.treat.sceneSize = queueSize;

  if (isTime) {
    gm.endTime = gm.startTime + gm.durationMs;
  } else {
    gm.endTime = 0;  // cards-bound ends when queue is exhausted
  }

  const seed = window.generateTreatmentScene
    ? window.generateTreatmentScene(queueSize, gm.difficulty)
    : [];
  gm.treat.queue = seed;

  showScreen('screen-gm-treat');
  treatRenderCard();
  treatRenderHud();
  treatRenderMiniZones();

  // Tick once a second to update the timer/elapsed cell, and end
  // the round if time-bound and the clock has run out.
  gm.treat.elapsedHandle = setInterval(treatTimerTick, 1000);
}

// Per-second ticker for the HUD's timer cell. In time-bound mode it
// also ends the round when the clock expires.
function treatTimerTick() {
  if (!gm.active) return;
  const isTime = (gm.bound === 'time');
  const el = document.getElementById('gm-treat-time');
  const lbl = document.getElementById('gm-treat-time-lbl');
  const cell = el ? el.parentElement : null;

  if (isTime) {
    const remainingMs = Math.max(0, gm.endTime - Date.now());
    const ts = Math.ceil(remainingMs / 1000);
    if (el)  el.textContent  = `${Math.floor(ts/60)}:${String(ts%60).padStart(2,'0')}`;
    if (lbl) lbl.textContent = 'Timer';
    if (cell) cell.classList.toggle('urgent', ts <= 15);
    if (Date.now() >= gm.endTime) gmEndGame();
  } else {
    const sec = Math.floor((Date.now() - gm.startTime) / 1000);
...
// Render the current patient (or end the round if queue exhausted).
function treatRenderCard() {
  const T = gm.treat;
  if (T.currentIndex >= T.queue.length) {
    gmEndGame();
    return;
  }
  const p = T.queue[T.currentIndex];
  const s = p.scenario;

  // First-unit hint banner
  const tagInfo = window.TRIAGE_INFO ? window.TRIAGE_INFO[p.initialTag] : null;
  const tagLabel = tagInfo ? tagInfo.label : (p.initialTag || '').toUpperCase();
  const tagWord = ({ red:'IMMEDIATE', yellow:'DELAYED', green:'MINOR', black:'DECEASED' })[p.initialTag] || '';
  const hintEl = document.getElementById('gm-treat-hint-color');
  if (hintEl) {
    hintEl.textContent = `${tagLabel} · ${tagWord}`;
    hintEl.className = 'hint-color tag-' + p.initialTag;
  }
  const hintBanner = document.getElementById('gm-treat-hint');
  if (hintBanner) hintBanner.dataset.tag = p.initialTag;

  // Card narrative + vitals
  const narEl = document.getElementById('gm-treat-narrative');
  if (narEl) narEl.textContent = s.description || '—';

  const vitalsEl = document.getElementById('gm-treat-vitals');
  if (vitalsEl) {
    vitalsEl.innerHTML = `
      <div class="vital"><span class="k">RESPIRATIONS</span><span class="v">${escapeHtml(s.respirations)}</span></div>
      <div class="vital"><span class="k">PERFUSION</span><span class="v">${escapeHtml(s.perfusion)}</span></div>
      <div class="vital"><span class="k">MENTAL STATUS</span><span class="v">${escapeHtml(s.mental)}</span></div>
    `;
  }

  // Reset verdict + show tag row
  const v = document.getElementById('gm-treat-verdict');
  if (v) v.hidden = true;
  const row = document.getElementById('gm-treat-tagrow');
  if (row) row.style.display = '';
  // Re-enable tag buttons (in case they were locked from previous card)
  qsa('#gm-treat-tagrow .t-btn').forEach(b => b.disabled = false);
}

// Player picked a tag for the current patient.
function treatPickTag(picked) {
  const T = gm.treat;
  if (T.currentIndex >= T.queue.length) return;
  const p = T.queue[T.currentIndex];
  const correct = (picked === p.correctTag);

  // Lock the tag row so accidental double-taps don't fire twice
  qsa('#gm-treat-tagrow .t-btn').forEach(b => b.disabled = true);

  // Classify the action for the AAR
  let kind;
  if (correct && p.initialTag === p.correctTag)         kind = 'agreed_correct';
  else if (correct && p.initialTag !== p.correctTag)    kind = 'fixed_mistag';
  else if (!correct && picked === p.initialTag)         kind = 'agreed_with_mistag'; // trusted a wrong first unit
  else if (!correct && p.initialTag === p.correctTag)   kind = 'broke_correct';     // patient was correctly tagged, you changed it wrong
  else                                                  kind = 'still_wrong';        // first unit was wrong, you picked a different wrong tag
  ...
  // Place the patient at the chosen tarp (regardless of correctness —
  // the player put them there, that's where they go on the AAR layout).
  T.placements[picked].push({
    patient: p,
    correct,
    kind,
  });
  ...
  treatShowVerdict(p, picked, correct, kind);
  treatRenderHud();
  treatRenderMiniZones();
}

function treatShowVerdict(p, picked, correct, kind) {
  const v       = document.getElementById('gm-treat-verdict');
  const head    = document.getElementById('gm-treat-verdict-head');
  const sub     = document.getElementById('gm-treat-verdict-sub');
  const rat     = document.getElementById('gm-treat-verdict-rationale');
  const row     = document.getElementById('gm-treat-tagrow');
  if (!v || !head || !sub || !rat) return;

  if (correct) {
    head.textContent = (kind === 'fixed_mistag') ? 'CORRECT — FIXED THE MISTAG' : 'CORRECT';
    head.className = 'verdict-head ok';
  } else {
    head.textContent = 'WRONG';
    head.className = 'verdict-head bad';
  }

  const t = (k) => (window.TRIAGE_INFO ? (window.TRIAGE_INFO[k]?.label) : (k||'').toUpperCase()) || k;
  const subParts = [];
  if (correct) {
    if (kind === 'fixed_mistag') subParts.push(`First unit said ${t(p.initialTag)} → you correctly retagged ${t(p.correctTag)}.`);
    else subParts.push(`Truth: ${t(p.correctTag)}.`);
  } else {
    subParts.push(`You picked ${t(picked)}. Truth: ${t(p.correctTag)}.`);
    if (kind === 'agreed_with_mistag') subParts.push(`First unit also tagged ${t(p.initialTag)} — anchoring bias.`);
    if (kind === 'broke_correct')      subParts.push(`First unit had it right at ${t(p.initialTag)}.`);
  }
  sub.textContent = subParts.join(' ');

  rat.textContent = p.scenario.rationale || '';
...
function treatNextPatient() {
  gm.treat.currentIndex++;
  treatRenderCard();
}

function treatRenderHud() {
  const T = gm.treat;
  const isTime = (gm.bound === 'time');
  const idx = Math.min(T.currentIndex + 1, T.queue.length);
  const setIf = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
  // Time-bound: just the running count (queue is huge, x/100 is meaningless).
  // Cards-bound: x of N, since N is the round target.
  setIf('gm-treat-progress', isTime ? String(idx) : `${idx}/${T.queue.length}`);
  setIf('gm-treat-correct',  T.correct);
  setIf('gm-treat-wrong',    T.wrong);
}

function treatRenderMiniZones() {
  const T = gm.treat;
  ['red','yellow','green','black'].forEach(z => {
    const el = document.getElementById('gm-treat-mini-' + z);
    if (el) el.textContent = T.placements[z].length;
  });
}
...
function gmRenderPatient(p, isNew) {
  ...
  el.innerHTML = `
    ...
    ${gmAirwayRowHtml(p)}
    <div class="gm-tagrow">
      <button class="gm-tag red"    data-tag="red">IMMEDIATE</button>
      <button class="gm-tag yellow" data-tag="yellow">DELAYED</button>
      <button class="gm-tag green"  data-tag="green">MINOR</button>
      <button class="gm-tag black"  data-tag="black">DECEASED</button>
    </div>
  `;
...
function gmEndGame() {
  ...
  if (gm.spawnHandle) { clearTimeout(gm.spawnHandle);  gm.spawnHandle = null; }
  if (gm.endHandle)   { clearTimeout(gm.endHandle);    gm.endHandle = null; }
  // Treatment Area uses its own AAR shape (queue + final treatment-area layout)
  if (gm.subMode === 'treat') {
    if (gm.treat && gm.treat.elapsedHandle) {
      clearInterval(gm.treat.elapsedHandle);
      gm.treat.elapsedHandle = null;
    }
    gmRenderTreatResults();
  ...
  // (with misses-review). Then if I'm the LAST player to finish,
  // self-trigger the lobby status flip so everyone routes to the
  // unified leaderboard. This matches Training mode's belt-and-
  // suspenders pattern in endGame() — the instructor's listener
  // also fires this, but Game Mode without a self-trigger has been
  // observed to stall when listeners race or get throttled at scale.
  if (gm.classroomMode && state.mpCode && state.mpMyId) {
    const updates = {};
    updates[`players/${state.mpMyId}/correct`] = gm.saveCount || 0;
    updates[`players/${state.mpMyId}/wrong`]   = (gm.triagedCount + gm.lostCount) - (gm.saveCount || 0);
    ...
    // Last-player end-of-round trigger. Treat ME as finished (Firebase
    // write above is racing the listener). If every other non-instructor
    // player is already finished, flip status — idempotent, multiple
    // racers all hit the same value.
    const allDone = (state.mpPlayers || []).every(p => {
      if (p.isInstructor) return true;
      if (p.id === state.mpMyId) return true;
      return !!p.finished;
...
function gmRenderResults() {
  ...
  // Visual treatment area — 4-zone visualization of every patient
  // tagged. Clears the container in modes where there's nothing useful
  // to show (eg. no history, or treatment-area mode which renders its
  // own layout in gmRenderTreatResults).
  const txEl = $('gm-tx-area');
  if (txEl) {
    txEl.innerHTML = renderTreatmentAreaHtml(getMyDecisions());
  }
  ...
  if (top.length === 0) {
    debriefEl.innerHTML = `<div class="gm-debrief-empty">Nothing to debrief. Frankly disappointing for the instructor.</div>`;
  } else {
...
// Treatment Area AAR — training-style. Uses correct/wrong counts and
// shows the final treatment-area layout (where the player placed each
// patient) plus per-card mistake review.
function gmRenderTreatResults() {
  const T = gm.treat || freshTreatState();
  const elapsedSec = Math.round((Date.now() - gm.startTime) / 1000);
  const m = Math.floor(elapsedSec / 60);
  const s = elapsedSec % 60;

  const total      = T.history.length;            // number of patients answered
  const correct    = T.correct;
  const wrong      = T.wrong;
  const acc        = total > 0 ? Math.round((correct / total) * 100) : 0;
  const fixed      = T.history.filter(h => h.kind === 'fixed_mistag').length;
  const anchored   = T.history.filter(h => h.kind === 'agreed_with_mistag').length;
  const broke      = T.history.filter(h => h.kind === 'broke_correct').length;
  ...
  const ranks = {
    S: 'TRIAGE COMMANDER',
    A: 'TREATMENT-AREA OFFICER',
    B: 'COMPETENT REASSESSMENT',
    C: 'NEEDS REPS',
    D: 'NEEDS REPS',
    F: 'RETURN TO TRAINING',
  };
  ...
  $('gm-grade').className = 'grade-badge g-' + (grade === 'S' ? 'A' : grade);
  $('gm-rank').textContent = ranks[grade];
  $('gm-final-score').textContent = correct;
  $('gm-final-line').textContent =
    `${m}:${String(s).padStart(2,'0')} elapsed · ${total}/${T.queue.length} reassessed · ${fixed} mistags fixed`;

  // Stat strip — repurpose existing tiles
  $('gm-acc').textContent    = acc + '%';
  $('gm-saved').textContent  = fixed;
  $('gm-deaths').textContent = anchored + broke;
  qsa('#screen-gm-results .stat-cell').forEach((cell, i) => {
    const lbl = cell.querySelector('.l');
    if (!lbl) return;
    if (i === 0) lbl.textContent = 'Accuracy';
    if (i === 1) lbl.textContent = 'Mistags Fixed';
    if (i === 2) lbl.textContent = 'Wrong Calls';
  });

  // Grade quip
  const roastEl = $('gm-roast');
  if (roastEl) {
    const pool = (typeof GRADE_QUIPS !== 'undefined') ? GRADE_QUIPS[grade] : null;
    if (pool && pool.length) {
      roastEl.textContent = pool[Math.floor(Math.random() * pool.length)];
      roastEl.className = 'result-roast grade-' + grade;
      roastEl.style.display = '';
  ...
  // Treatment Area mode renders its visualization inside the debrief
  // block, so clear the shared gm-tx-area container to avoid showing
  // the viz twice.
  const txEl = $('gm-tx-area');
  if (txEl) txEl.innerHTML = '';

  // Build the after-action layout: final treatment-area layout (where
  // patients were placed) + a mistakes review.
  const debrief = $('gm-debrief');
  if (!debrief) return;

  // Use the shared treatment-area renderer so the AAR matches every
  // other mode (heading, silhouette pips, flashing wrong placements,
  // click-for-why).
  const layout = renderTreatmentAreaHtml(getMyDecisions());
  const mistakes = T.history.filter(h => !h.correct);
  let mistakesHtml = '';
  if (mistakes.length === 0) {
    mistakesHtml = `<div class="gm-debrief-empty">Clean run. Either you trained hard or you cheated. We\'re voting cheated.</div>`;
  } else {
    mistakesHtml = `<h3 class="treat-debrief-h">MISSED CALLS · ${mistakes.length}</h3>` +
      mistakes.map(h => treatMistakeCard(h)).join('');
  }
  // Highlight fixes too (positive reinforcement)
  const fixes = T.history.filter(h => h.kind === 'fixed_mistag');
  let fixesHtml = '';
  if (fixes.length) {
    fixesHtml = `<h3 class="treat-debrief-h treat-pos">FIRST-UNIT MISTAGS YOU FIXED · ${fixes.length}</h3>` +
      fixes.map(h => treatFixCard(h)).join('');
  }

  debrief.innerHTML = layout + mistakesHtml + fixesHtml;
}

function treatMistakeCard(h) {
  const t = (k) => (window.TRIAGE_INFO ? window.TRIAGE_INFO[k]?.label : (k || '').toUpperCase()) || k;
  const p = h.patient;
  const head =
    (h.kind === 'agreed_with_mistag') ? `ANCHORED ON FIRST UNIT — agreed with ${t(p.initialTag)}, truth ${t(p.correctTag)}` :
    (h.kind === 'broke_correct')      ? `RETAGGED FROM ${t(p.initialTag)} (correct) → ${t(h.picked)} (wrong)` :
                                         `PICKED ${t(h.picked)} — truth was ${t(p.correctTag)}`;
  return `<div class="gm-debrief-card lost">
    <div class="gm-debrief-head">${head}</div>
    <div class="gm-debrief-body">${escapeHtml(p.scenario.description)}</div>
    <div class="gm-debrief-meta">${escapeHtml(p.scenario.rationale || '')}</div>
  </div>`;
}

function treatFixCard(h) {
  const t = (k) => (window.TRIAGE_INFO ? window.TRIAGE_INFO[k]?.label : (k || '').toUpperCase()) || k;
  const p = h.patient;
  return `<div class="gm-debrief-card ok">
    <div class="gm-debrief-head">FIXED: ${t(p.initialTag)} → ${t(p.correctTag)} ✓</div>
    <div class="gm-debrief-body">${escapeHtml(p.scenario.description)}</div>
    <div class="gm-debrief-meta">${escapeHtml(p.scenario.rationale || '')}</div>
  </div>`;
}
...
function gmInitWiring() {
  ...
  // starts the run.
  const gmModeButtons = ['gm-mode-chaos', 'gm-mode-fog', 'gm-mode-treat'];
  const gmModeBlurbs = {
    ...
    treat: 'TREATMENT AREA · Patients arrive ALREADY tagged by a first unit. Work through them one at a time — reassess each, agree or retag, place at the correct tarp. Honors the round-type / length picker above. AAR shows your final treatment-area layout.',
  };
  ...
  $('gm-start')?.addEventListener('click', () => {
    if (!['chaos', 'fog', 'treat'].includes(gm.subMode)) {
  ...
  // and skips straight to the pick phase. Player gets the normal pick
  // window (6-9s) — clicking just removes the wait time, not the
  // thinking time. The airway button handles its own click and bubbles,
  // so we ignore those — only the card surface itself is the skip trigger.
  ...
  // ---- Treatment Area controls (queue / training-style) ----
  // Tag buttons — pick a tag for the current patient
  qsa('#gm-treat-tagrow .t-btn').forEach(btn => {
    btn.addEventListener('click', () => treatPickTag(btn.dataset.tag));
  });
  // NEXT PATIENT — advance the queue
  $('gm-treat-next')?.addEventListener('click', treatNextPatient);
  // End drill early
  $('gm-treat-end-early')?.addEventListener('click', () => {
    if (!gm.active) return;
    if (confirm('End the drill now?')) gmEndGame();
  });
...
/* ============================================================
   REPORT A BUG — opens from the home footer link.
   Submits to a form-to-email service so the destination email
   stays hidden (configured on the service side, not in the HTML).
   Default integration is Web3Forms (free, unlimited). To activate:
     1. Sign up at https://web3forms.com (free, takes 30 seconds).
     2. Enter andrewh82790@gmail.com as the destination.
     3. Copy the access key they email you.
     4. Replace REPORT_BUG_ACCESS_KEY below with that key.
   ============================================================ */

// Replace this with your Web3Forms access key. Until then, sends fail
// gracefully with a "not configured" message — nothing reaches the user.
const REPORT_BUG_ACCESS_KEY = 'f98e8f99-8f67-4c3f-915f-482768be5e20';

function openReportBugModal() {
  const overlay = document.getElementById('report-bug-modal');
  if (!overlay) return;
  const status = document.getElementById('report-bug-status');
  if (status) { status.textContent = ''; status.className = 'report-bug-status'; }
  overlay.hidden = false;
  setTimeout(() => {
    const ta = document.getElementById('report-bug-message');
    if (ta) ta.focus();
  }, 100);
}
...
function setReportBugStatus(msg, kind) {
  const status = document.getElementById('report-bug-status');
  if (!status) return;
  status.textContent = msg;
  status.className = 'report-bug-status' + (kind ? ' ' + kind : '');
}

async function sendReportBug() {
  const messageEl = document.getElementById('report-bug-message');
  const nameEl    = document.getElementById('report-bug-name');
  const modeEl    = document.getElementById('report-bug-mode');
  const sendBtn   = document.getElementById('report-bug-send');
  if (!messageEl || !sendBtn) return;
  const message = (messageEl.value || '').trim();
  if (message.length < 5) {
    setReportBugStatus('Please give a detailed description.', 'error');
    messageEl.focus();
    return;
  }
  if (!REPORT_BUG_ACCESS_KEY || REPORT_BUG_ACCESS_KEY === 'YOUR_WEB3FORMS_KEY_HERE') {
    setReportBugStatus('Bug reporting isn\'t configured yet. (Owner: see REPORT_BUG_ACCESS_KEY in app.js.)', 'error');
    return;
  }
  const name = (nameEl?.value || '').trim() || 'Anonymous';
  const mode = (modeEl?.value || '').trim() || 'Not specified';

  // Only the fields the user filled in. No auto-attached app context.
  const payload = {
    access_key: REPORT_BUG_ACCESS_KEY,
    subject: `Triage It — Bug Report from ${name}`,
    from_name: name,
    name,
    mode,
    message,
  ...
  try {
    ...
    if (res.ok && data.success !== false) {
      setReportBugStatus('Thanks — report sent. Will look into it.', 'success');
      messageEl.value = '';
      if (nameEl) nameEl.value = '';
      if (modeEl) modeEl.value = '';
      setTimeout(closeReportBugModal, 1500);
    } else {
      throw new Error((data && data.message) || `HTTP ${res.status}`);
    }
  } catch (e) {
    setReportBugStatus('Send failed. Check your connection and try again.', 'error');
  } finally {
...
function initReportBug() {
  document.getElementById('report-bug-open')?.addEventListener('click', openReportBugModal);
  document.getElementById('report-bug-topbar')?.addEventListener('click', openReportBugModal);
  document.getElementById('report-bug-close')?.addEventListener('click', closeReportBugModal);
  document.getElementById('report-bug-cancel')?.addEventListener('click', closeReportBugModal);
  document.getElementById('report-bug-send')?.addEventListener('click', sendReportBug);
  document.getElementById('report-bug-modal')?.addEventListener('click', (e) => {
    if (e.target.id === 'report-bug-modal') closeReportBugModal();
  });
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initReportBug);
} else {
  initReportBug();
}

/* ============================================================
   DEBUG MODE — bot simulator UI (URL ?debug=1)
   ============================================================
   When the URL carries ?debug=1, reveal anything tagged
   data-debug-only and wire the lobby's bot-simulator buttons
   to window.stress() / window.stop() (provided by stress-test.js).
   Bots run server-side via Firebase writes — they don't render
   any UI of their own, so use this for dashboard / leaderboard
   load testing, not for finding solo-mode bugs.
   ============================================================ */
function initDebugMode() {
  const params = new URLSearchParams(window.location.search);
  if (params.get('debug') !== '1') return;

  // Unhide every debug-only element
  qsa('[data-debug-only]').forEach(el => { el.hidden = false; });

  const status = (msg) => {
    const el = document.getElementById('debug-bots-status');
    if (el) el.textContent = msg;
  };

  function addBots(n) {
    if (typeof window.stress !== 'function') {
      status('Bot simulator not loaded. Check stress-test.js.');
      return;
    }
    if (!state.mpCode) {
      status('No lobby code yet. Host a drill first.');
      return;
    }
    if (state.mpRole !== 'host' && state.mpRole !== 'instructor') {
      status('Only the host can add bots.');
      return;
    }
    // window.stress refuses to start a second batch while one is active —
    // call stop() first so repeated clicks are forgiving.
    if (typeof window.stop === 'function') window.stop();
    setTimeout(() => {
      window.stress(state.mpCode, n);
      status(`${n} bots added. Click START GAME to watch them tick.`);
    }, 50);
  }

  document.getElementById('debug-add-bots-5')?.addEventListener('click', () => addBots(5));
  document.getElementById('debug-add-bots-10')?.addEventListener('click', () => addBots(10));
  document.getElementById('debug-add-bots-25')?.addEventListener('click', () => addBots(25));
  document.getElementById('debug-stop-bots')?.addEventListener('click', () => {
    if (typeof window.stop === 'function') {
      window.stop();
      status('All bots removed.');
    }
  });

  console.log('[debug] Debug mode active. Bot tools visible in lobby.');
}
