...
function onTagPick(tag) {
  ...
  // Earlier this was a 1500ms auto-timer, but during multiplayer rounds
  // people did not have enough time to read the rationale.
  const cardEl = $('card');
  const advance = () => {
  ...
  // Slight delay so the flip animation can play before clicks register
  setTimeout(() => {
...
function showReviewScreen() {
  const deck = state.mpDeck;
  if (!deck || !deck.length) {
    toast('No deck data to review');
    return;
  }
  // Aggregate misses across all players, grouped by card index
  const byCard = {};
  state.mpPlayers.forEach(p => {
    (p.misses || []).forEach(m => {
    ...
    html += `
      <details class="review-card">
        <summary class="review-summary">
          ...
          <span class="review-misscount">${missedBy.length} miss${missedBy.length > 1 ? 'es' : ''}</span>
