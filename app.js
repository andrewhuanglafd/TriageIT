  ...
  qsa('#host-gametype-options [data-gametype]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (gt === 'training') {
        ...
        // Game-mode types don't support team scoring — each player
        // runs their own scene/timer. Force head-to-head and clear
        // any leftover team assignments from a prior Training round.
        state.mpMode = 'ffa';
      }
      ...
      if (state.mpCode && (state.mpRole === 'host' || state.mpRole === 'instructor')) {
        ...
        if (gt !== 'training') {
          updates.mode = 'ffa';
          // Strip any team assignments from active players
          state.mpPlayers.forEach(p => {
