...
const state = {
  ...
  mpMode: 'ffa',          // 'ffa' | 'team'
  mpDeckCount: 25,
  mpDifficulty: 'normal', // 'normal' | 'hard' | 'expert'
...
function buildDeck(count, difficulty) {
  ...
  //   'expert' — same hard scenarios, but sudden-death scoring (one wrong = over)
  // Both 'hard' and 'expert' use the same weighted scenario pool internally.
  const genDifficulty = (difficulty === 'hard' || difficulty === 'expert') ? 'veteran' : 'normal';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('sp-start').addEventListener('click', () => {
    ...
    // Read difficulty from radio group (defaults to normal if neither checked)
    const checked = document.querySelector('input[name="sp-difficulty"]:checked');
  ...
  // Difficulty radios (host only). Click a checked one to uncheck and return to Normal.
  qsa('input[name="mp-difficulty"]').forEach(radio => {
    radio.addEventListener('change', (e) => {
      setMpDifficulty(e.target.checked ? e.target.value : 'normal');
    });
    // Click-to-uncheck for radios — if user taps the active one, deselect
    radio.addEventListener('click', (e) => {
      if (radio.dataset.wasChecked === 'true') {
        radio.checked = false;
        radio.dataset.wasChecked = 'false';
        setMpDifficulty('normal');
      } else {
        // Mark this one, unmark others
        qsa('input[name="mp-difficulty"]').forEach(r => r.dataset.wasChecked = 'false');
        radio.dataset.wasChecked = 'true';
      }
    });
  });
  // Same for solo
  qsa('input[name="sp-difficulty"]').forEach(radio => {
    radio.addEventListener('click', () => {
      if (radio.dataset.wasChecked === 'true') {
        radio.checked = false;
        radio.dataset.wasChecked = 'false';
      } else {
        qsa('input[name="sp-difficulty"]').forEach(r => r.dataset.wasChecked = 'false');
        radio.dataset.wasChecked = 'true';
      }
    });
  });
...
function onTagPick(tag) {
  ...
  if ((g.mode === 'mp-host' || g.mode === 'mp-guest') && state.mpCode && state.mpMyId) {
    ...
    const expertFail = (!correct && g.difficulty === 'expert');
    ...
    // Triggers when host finishes the deck OR fails in Expert mode.
    const meDone = expertFail || (g.idx + 1 >= g.deck.length);
    if (g.mode === 'mp-host' && meDone) {
  ...
  const advance = (e) => {
    ...
    if (!correct && g.difficulty === 'expert') {
      g.expertFailed = true;
...
async function hostStartGame() {
  ...
  const checked = document.querySelector('input[name="mp-difficulty"]:checked');
  const difficulty = checked ? checked.value : 'normal';
...
function renderLobby() {
  ...
  $('guest-hard-indicator').style.display = (state.mpRole === 'guest' && state.mpDifficulty === 'hard') ? 'block' : 'none';
  $('guest-expert-indicator').style.display = (state.mpRole === 'guest' && state.mpDifficulty === 'expert') ? 'block' : 'none';
  ...
  // Sync host's difficulty radios to lobby state
  const diffHard = $('mp-difficulty-hard');
  const diffExpert = $('mp-difficulty-expert');
  if (diffHard) {
    diffHard.checked = (state.mpDifficulty === 'hard');
    diffHard.dataset.wasChecked = diffHard.checked ? 'true' : 'false';
  }
  if (diffExpert) {
    diffExpert.checked = (state.mpDifficulty === 'expert');
    diffExpert.dataset.wasChecked = diffExpert.checked ? 'true' : 'false';
  }
}
